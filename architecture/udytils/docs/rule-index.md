...
**This index is a lookup table, not a starting point.** To write ordinary code, copy the nearest existing example and run `./gradlew :udytils-architecture:test` — its failure output names the failed rule's ID and statement, and for membership and exhaustiveness failures lists the closest Constructs with a per-requirement checklist. Come here for an ID that has already failed, or when adding a Construct or designing a new subsystem.
...
| `CoreModule.standaloneInBuild` | The core module must not declare a Gradle dependency on any other project | [tested](../src/main/kotlin/dev/isaacudy/udytils/rules/core/CoreModule.kt) |
...
| `WebModules.standaloneModules` | The web family must not depend on any other udytils family, and its two modules must not depend on each other | [tested](../src/main/kotlin/dev/isaacudy/udytils/rules/web/WebModules.kt) |
...
| `AtlasModules.standaloneFamily` | The atlas family must not depend on any other udytils family | [tested](../src/main/kotlin/dev/isaacudy/udytils/rules/atlas/AtlasModules.kt) |
