    ...
    private fun writeGeneratedFile(
        ...
        codeGenerator.createNewFile(
            ...
            OutputStreamWriter(stream, Charsets.UTF_8).use { writer ->
                ...
                writer.appendLine("import dev.isaacudy.udytils.urpc.UrpcLogger")
                writer.appendLine("import dev.isaacudy.udytils.urpc.client.UrpcClientFactory")
                writer.appendLine("import dev.isaacudy.udytils.urpc.server.ServiceErrorMapper")
                writer.appendLine("import dev.isaacudy.udytils.urpc.server.installUnary")
                writer.appendLine("import io.ktor.server.routing.Route")
                ...
                writer.appendLine("fun Route.urpc(")
                writer.appendLine("    impl: $serviceFqn,")
                writer.appendLine("    type: KClass<$serviceFqn>,")
                writer.appendLine("    errorMapper: ServiceErrorMapper = ServiceErrorMapper.Default,")
                writer.appendLine("    logger: UrpcLogger = UrpcLogger.NoOp,")
                writer.appendLine(") {")
                for (d in descriptors) {
                    ...
                    writer.appendLine("        errorMapper = errorMapper,")
                    writer.appendLine("        logger = logger,")
